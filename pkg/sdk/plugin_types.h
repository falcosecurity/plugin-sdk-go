...
// falcosecurity libs types. Metadata types start at 1000000 in order
// to avoid conflicts with falcosecurity libs types.
typedef enum ss_plugin_field_type {
	...
	FTYPE_IPNET = 41,
	// A Go/Python-style zero-indexed start and end of the field's
	// location in the event data as a uint32_t pair. {0,0} can be used
	// by extractors to indicate that the field doesn't correspond to a
	// location.
	FTYPE_METADATA_SLICE = 1000000,
