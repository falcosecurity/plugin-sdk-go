...
extern int32_t plugin_extract_fields_sync(ss_plugin_t *s,
										  ...
										  ss_plugin_extract_field_offsets *offsets);
							  ...
							  const ss_plugin_field_extract_input* in)
{
	...
	{
		return plugin_extract_fields_sync(s, evt, in->num_fields, in->fields, in->field_offsets);
	}
	...
	s_async_ctx_batch[(size_t)s - 1].field_offsets = in->field_offsets;
